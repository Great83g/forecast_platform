...
def onboarding_guide(request):
    ...
    guide_steps = [
        {
            "title": tr("Добавьте первую станцию", "Бірінші станцияны қосыңыз"),
            "summary": tr(
                "Откройте раздел «Станции» и нажмите кнопку «Добавить станцию».",
                "«Станциялар» бөліміне өтіп, «Станция қосу» батырмасын басыңыз.",
            ),
            "details": [
                tr("Заполните название станции, мощность, координаты и организацию.", "Станция атауын, қуатын, координаттарын және ұйымын толтырыңыз."),
                tr("После сохранения проверьте, что станция появилась в общем списке.", "Сақтағаннан кейін станцияның жалпы тізімде пайда болғанын тексеріңіз."),
            ],
        ...
        {
            "title": tr("Заполните параметры станции", "Станция параметрлерін толтырыңыз"),
            "summary": tr(
            ...
            "details": [
                tr("Сверьте технические параметры перед сохранением карточки станции.", "Станция картасын сақтар алдында техникалық параметрлерді тексеріңіз."),
                tr("Если есть несколько станций, используйте понятные названия для каждой из них.", "Егер бірнеше станция болса, әрқайсысына түсінікті атау беріңіз."),
            ],
        ...
        {
            "title": tr("Загрузите исторические данные", "Тарихи деректерді жүктеңіз"),
            "summary": tr(
                "Перейдите в карточку станции и откройте раздел загрузки истории.",
                "Станция картасына өтіп, тарихты жүктеу бөлімін ашыңыз.",
            ),
            "details": [
                tr("Подготовьте файл с историей генерации в нужном формате.", "Генерация тарихының файлын қажетті форматта дайындаңыз."),
                tr("После импорта убедитесь, что данные появились без ошибок.", "Импорттан кейін деректер қателіксіз жүктелгенін тексеріңіз."),
                tr("Если истории пока нет, этот шаг можно пропустить и использовать режим прогноза без истории.", "Егер тарих әзірге жоқ болса, бұл қадамды өткізіп, тарихсыз болжам режимін пайдаланыңыз."),
            ],
        ...
        {
            "title": tr("Обучите модель и запустите прогноз", "Модельді оқытып, болжамды іске қосыңыз"),
            "summary": tr(
            ...
            "details": [
                tr("После импорта истории обязательно запустите обучение модели для станции.", "Тарих импортталғаннан кейін станция үшін модельді оқытуды міндетті түрде іске қосыңыз."),
                tr("Дождитесь завершения обучения и проверьте статус доступных моделей.", "Оқыту аяқталғанша күтіп, қолжетімді модельдердің күйін тексеріңіз."),
                tr("Затем выполните расчёт прогноза и проверьте итоговые значения.", "Одан кейін болжамды есептеп, қорытынды мәндерді тексеріңіз."),
                tr("Если истории нет, запустите режим прогноза без истории (только эвристика).", "Егер тарих жоқ болса, тарихсыз болжам режимін іске қосыңыз (тек эвристика)."),
            ],
        ...
        {
            "title": tr("Проверьте отчёт и доступы", "Есеп пен қолжетімділікті тексеріңіз"),
            "summary": tr(
            ...
            "details": [
                tr("Проверьте почасовой прогноз, отчёт и экспортируемые данные.", "Сағаттық болжамды, есепті және экспортталатын деректерді тексеріңіз."),
                tr("Выдайте доступ коллегам, если они тоже будут работать со станцией.", "Егер әріптестер де станциямен жұмыс істесе, оларға қолжетімділік беріңіз."),
            ],
    ...
    support_steps = [
        tr("Создайте станцию с корректными параметрами мощности и координат.", "Қуат пен координат параметрлері дұрыс станцияны жасаңыз."),
        tr("Загрузите историю генерации (если есть) через раздел «Загрузить историю».", "Генерация тарихын (бар болса) «Тарихты жүктеу» бөлімі арқылы жүктеңіз."),
        tr("После загрузки истории обучите модели, затем запустите прогноз.", "Тарих жүктелгеннен кейін модельдерді оқытып, содан соң болжамды іске қосыңыз."),
        tr("Если истории нет, используйте прогноз без истории (эвристический режим).", "Егер тарих жоқ болса, тарихсыз болжамды пайдаланыңыз (эвристикалық режим)."),
    ]
    return render(
        request,
        "dashboard/onboarding_guide.html",
        {
            ...
            "guide_eyebrow": tr("Гид", "Нұсқаулық"),
            "guide_title": tr("Как работать в платформе", "Платформада қалай жұмыс істеу керек"),
            "guide_lead": tr(
            ...
            "quick_start_title": tr("Быстрый старт", "Жылдам бастау"),
            "quick_start_text": tr("Откройте раздел «Станции», добавьте объект и перейдите к загрузке истории.", "«Станциялар» бөліміне өтіп, объект қосып, тарих жүктеуге өтіңіз."),
            "quick_start_button": tr("Добавить станцию", "Станция қосу"),
            "steps_eyebrow": tr("Шаги", "Қадамдар"),
            "steps_title": tr("Основной порядок действий", "Негізгі жұмыс тәртібі"),
            "tips_eyebrow": tr("Подсказки", "Кеңестер"),
            "tips_title": tr("Что важно не пропустить", "Маңызды нәрселер"),
            "guide_note_text": tr("Если нужна помощь, перейдите в раздел «Контакты» и свяжитесь с нами удобным способом.", "Көмек қажет болса, «Байланыс» бөліміне өтіп, бізбен ыңғайлы тәсілмен хабарласыңыз."),
        },
    )
