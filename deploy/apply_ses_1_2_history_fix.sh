...
if [ -z "$STATION_ID" ]; then
  STATION_ID="$(python manage.py shell -c "from stations.models import Station; qs=Station.objects.filter(name__icontains='1.2').order_by('id'); jez=qs.filter(name__icontains='JezSolar'); qs=jez if jez.exists() else qs; print(qs.first().id if qs.exists() else '')")"
fi
if [ -z "$STATION_ID" ]; then
...
python manage.py shell -c "from solar.models import SolarRecord; from stations.models import Station; s=Station.objects.get(id=$STATION_ID); qs=SolarRecord.objects.filter(station=s, timestamp__date='$HISTORY_DATE').order_by('timestamp'); print('rows=', qs.count()); print('---'); print('\\n'.join(f'{r.timestamp:%Y-%m-%d %H:%M} | power_kw={r.power_kw}' for r in qs))"
